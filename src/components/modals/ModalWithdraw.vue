      ...
      <div class="col-8 col form-section-body">
        <form>
          ...
          <div class="row row-list-input mb-1">
            <div class="col">
              <input placeholder="Amount" v-model.trim="withdrawAmount" />
              <div class="max" @click="withdrawAmount = pool.maxWithdraw">
                <span>max</span>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col d-flex justify-content-start p-0">
              ...
              >
                <small>Available max {{ pool.deposited }}</small>
                <small
...
export default {
  ...
  methods: {
    handleWithdraw() {
      this.$emit("withdraw", { pool: this.pool, amount: this.withdrawAmount });
    },
    getImgUrl(pic) {
