...
const POOLS_INFO = {
  ...
  "TEMP-SUSHI-POOL": {
    ...
    links: {
      ETH:
        "https://etherscan.io/address/0xbf513ace2abdc69d38ee847effdaa1901808c31c#code",
      BSC:
        "https://bscscan.com/address/0xbf513ace2abdc69d38ee847effdaa1901808c31c#code",
      FTM:
        "https://ftmscan.com/address/0xbf513ace2abdc69d38ee847effdaa1901808c31c#code",
    },
    lpsLink: {
      ETH:
        "https://app-uniswap-org.ipns.dweb.link/#/add/v2/ETH/0xf16e81dce15B08F326220742020379B855B87DF9",
      BSC:
  ...
  "TEMP-LP-POOL": {
    ...
    lpsLink: {
      ETH:
        "https://app-uniswap-org.ipns.dweb.link/#/add/v2/ETH/0xf16e81dce15B08F326220742020379B855B87DF9",
      BSC: "https://app.sushi.com/add/ETH/0xf16e81dce15B08F326220742020379B855B87DF9",
      FTM:
        "https://app.sushi.com/add/ETH/0xf16e81dce15B08F326220742020379B855B87DF9",
    },
    processingDeposit: false,
  ...
  "TEMP-SUSHI-POOL-1": {
    ...
    lpsLink: {
      ETH:
        "https://app.sushi.com/add/ETH/0xf16e81dce15B08F326220742020379B855B87DF9",
      BSC:
        "https://exchange.pancakeswap.finance/#/add/BNB/0xf16e81dce15B08F326220742020379B855B87DF9",
      FTM:
  ...
  "TEMP-LP-POOL-1": {
    ...
    lpsLink: {
      ETH:
        "https://app.sushi.com/add/ETH/0xf16e81dce15B08F326220742020379B855B87DF9",
      BSC: "https://exchange.pancakeswap.finance/#/add/BNB/0xf16e81dce15B08F326220742020379B855B87DF9",
      FTM:
        "https://spookyswap.finance/add/0xf16e81dce15B08F326220742020379B855B87DF9/FTM",
    },
    processingDeposit: false,
...
export default class Pool {
  ...
  getYield = async (amount = 1000) => {
    try {
      const divide = +fromWei(this.stakingTokenTotalAmount, this.weiFormat) + +amount;

      const multiplier = 86400;

      const icePerSecond = +await this.contract.instance.methods.icePerSecond().call();

      const totalAllocPoint = await this.contract.instance.methods.totalAllocPoint().call();

      let iceReward =
        (+multiplier * +icePerSecond * +this.allocPoint) /
        +totalAllocPoint;
      ...
      const accIcePerShare = 
        +this.accIcePerShare +
          (+iceReward * Math.pow(10, 12)) / +divide;

      const rewardDebt =
        (+amount * +this.accIcePerShare) /
        Math.pow(10, 12);

      const pending = 
        (+amount * +accIcePerShare) / Math.pow(10, 12) -
          +rewardDebt;

      const result = parseFloat(fromWei(pending.toLocaleString("fullwide", { useGrouping: false }), null)).toFixed(2);
      return result;
    } catch (error) {
      console.log(error);
    }
  ...
  getLPYield = async () => {
    try {
      ...
      let IceInSlpTotal = +await iceInstance.methods.balanceOf(stakingToken).call();

      let totalTokensSLPMinted = +await erc20.methods.totalSupply().call();
      ...
      const final = await this.getYield(res);
  ...
  getPoolYield = async () => {
    try {
      ...
      if (this.name === "TEMP-LP-POOL" || this.name === "TEMP-SUSHI-POOL") {
        poolYield = await this.getLPYield();
        this.yieldPerDollar = parseFloat(poolYield).toFixed(2);
        await this.getRoi(poolYield);
      }

      if (this.name === "TEMP-LP-POOL-1" || this.name === "TEMP-SUSHI-POOL-1") {
        poolYield = await this.getLPYield();
        this.yieldPerDollar = parseFloat(poolYield).toFixed(2);
        await this.getRoi(poolYield);
      }

      if (this.name === "TEMP-LP-POOL-2" || this.name === "TEMP-SUSHI-POOL-2") {
        poolYield = await this.getLPYield();
        this.yieldPerDollar = parseFloat(poolYield).toFixed(2);
        await this.getRoi(poolYield);
      }
  ...
  getPoolTvl = async () => {
    try {
      ...
      if (this.name === "TEMP-LP-POOL") {
        tvl = await this.getLPTvl();
      }

      if (this.name === "TEMP-LP-POOL-1") {
        tvl = await this.getLPTvl();
      }

      if (this.name === "TEMP-LP-POOL-2") {
        tvl = await this.getLPTvl();
      }
      ...
      if (this.name === "TEMP-SUSHI-POOL") {
        tvl = await this.getLPTvl();
      }

      if (this.name === "TEMP-SUSHI-POOL-1") {
        tvl = await this.getLPTvl();
      }

      if (this.name === "TEMP-SUSHI-POOL-2") {
        tvl = await this.getLPTvl();
      }
